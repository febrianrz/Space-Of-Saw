...
class Mprediksi extends CI_Model
{
	...
	public function getTotalPeriode($start_date, $end_date, $type){
		...
		if($type == 'days'){
			...
			// echo $inteval->format('%a');die();
			return floor(((int)$inteval->format('%a'))/6);		
		} else if($type == 'months') {
	...
	public function getTanggalByPeriode($periode,$interval){
	...
	public function getSingleItemPrediksi($id_produk, $start_date, $end_date, $no, $totalPeriode, $typeInterval){
		...
		$totalX =  array_sum(range(1,$totalPeriode));
		...
		$mape = $this->getTotalMape($totalGalat, $x);
		$data = (object) [
			'tanggal'	=> $start_date,

...
class PrediksiSemuaKue extends CI_Controller {
	...
	public function index2()
	{
		...
		foreach ($aaa as $kueid)
		{
			...
			// $data['graf_pre']=$prediksi;
			// $data['graf_kue']=$nama_produk;
	...
	public function index(){
		$data['typeInterval']	= "days"; //days, weeks
		$data['start_date'] 	= $this->input->get('start')?date('Y-m-d', strtotime($this->input->get('start'))):$this->Mprediksi->getFirstDate();
		$data['end_date'] 		= $this->input->get('end')?date('Y-m-d', strtotime($this->input->get('end'))):$this->Mprediksi->getLastDate();
		$data['maxPeriode']		= $this->Mprediksi->getTotalPeriode($data['start_date'], $data['end_date'],$data['typeInterval']);
		$data['periodeDicari']	= $data['maxPeriode']+1;
 ...
 //       		// echo $kueid['id_produk'];
	...
	public function detail(){
		...
		if($id_produk==0) return show_404();
		
		$data['start_date'] 	= $this->input->get('start')?date('Y-m-d',strtotime($this->input->get('start'))):$this->Mprediksi->getFirstDate();
		$data['end_date'] 		= $this->input->get('end')?date('Y-m-d',strtotime($this->input->get('end'))):$this->Mprediksi->getLastDate();
		$data['typeInterval']	= $this->input->get('typeInterval')?$this->input->get('typeInterval'):'days'; //days atau weeks
		$data['totalPeriode']	= $this->Mprediksi->getTotalPeriode($data['start_date'], $data['end_date'], $data['typeInterval']);
		$data['totalX']			= array_sum(range(1,$data['totalPeriode']));
		$data['periodeDicari']	= $data['totalPeriode']+1;
		// var_dump($data);die();
		$results = [];
		$tmp_start = $data['start_date'];
		if($data['typeInterval'] == 'weeks'){
			$tmp_end   = date('Y-m-d',strtotime($tmp_start." +6 days"));
		} else {
			if($data['typeInterval'] != 'days'){
				$tmp_end   = date('Y-m-d',strtotime($tmp_start." +1 ".$data['typeInterval']));	
			} else {
				$tmp_end   = $tmp_start;
			}
		...
		// echo $data['totalPeriode'];die();
		for($i=0;$i<$data['totalPeriode'];$i++){
			
			if($i != 0){
				$tmp_start = date('Y-m-d',strtotime($tmp_end." +1 days"));
			...
			$tmp = [
				'tanggal_awal'	=> $tmp_start,
				'tanggal_akhir' => $tmp_end,
				'total'			=> '',
				'time'			=> $i+1,
				'forecast'		=> 0,
				'error'			=> 0,
		...
		$cakes = $this->db->get_where('produk',['id_produk'=>$id_produk])->row();
		
		$data['produk']			= $cakes;
		$data['results']		= $results;
		// echo json_encode($data['prediksi']);die();

		
		// echo json_encode($data['prediksi'][$data['produk']->id_produk]);die();
		$data['n']				= 0;
		...
		$this->load->view('admin/vheader');
		$this->load->view('admin/vmenu');
		$this->load->view('admin/prediksi_semua/vprediksi_semuakue_perperiode_detail',$data);	
	}
